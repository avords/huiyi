...
public class CheckBoxTag extends SimpleTagSupport {
    private static final Logger LOGGER = Logger.getLogger(CheckBoxTag.class);
        ...
        for (Dictionary baseEnum : dictionaries) {
            if (baseEnum != null) {
                getJspContext().getOut().write(
                        "<label style=\"margin-right:20px;\"><input type=\"checkBox\" name=\"" + name + "\" id=\"" + name + baseEnum.getValue()
                        + "\" value=\"" + baseEnum.getValue() + "\" class=\"flat-red\"/>" + baseEnum.getName()
                        + "</label>");
            }
        }
    }
