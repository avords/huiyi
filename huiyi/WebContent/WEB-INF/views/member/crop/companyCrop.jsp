...
<title><jdf:message code="品牌管理" /></title>
...
<%--         <div id="preview"><img id="generated" src="${dynamicDomain}${filePath}"  /></div>  --%>
     ...
     $(function() {
          ...
          $("#crop").click(function(){//裁剪提交
              cropzoom.send('${dynamicDomain}/admin/companyScreenshot/saveCrop', 'POST', {'srcFilePath':'${filePath}'}, function(imgRet) {
                  ...
                  if('${type}'=='main'){
                      var img = '<a style="float: right;cursor: pointer;" id="mainDelete" data-path="'+imgRet.filePath+'">删除</a>'+
                   	   '<img alt="" src="'+filePath+'" width="100px" height="100px;">';
                      $("#mainPicture",window.parent.document).val(imgRet.filePath);
                      $("#mainImg",window.parent.document).html(img);
                  }
                  else if('${type}'=='logo'){
                      var img = '<a style="float: right;cursor: pointer;" id="logoDelete" data-path="'+imgRet.filePath+'">删除</a>'+
                   	   '<img alt="" src="'+filePath+'" width="100px" height="100px;">';
                      $("#logoAddress",window.parent.document).val(imgRet.filePath);
                      $("#logoImg",window.parent.document).html(img);
                  }
                  else if('${type}'=='sub'){
                      var img = '<div style="width: 100px;display: inline-block;">'+
                      '<a style="cursor: pointer;display: block;margin-left: 65px;" class="subDelete" data-path="'+imgRet.filePath+'">删除</a>'+
                      '<img alt="" src="'+filePath+'" width="100px" height="100px;"></div>';
