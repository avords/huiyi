...
public final class I18NUtils {
	 ...
	 * @throws RuntimeException
	 */
	public static void transformDict(Dictionary t, HttpServletRequest request) throws RuntimeException {
        if (t != null) {
            Locale locale = LocaleUtils.getLocale(request);
            t.setName(MessageUtils.getMessage(t.getName(), locale));
        }
    }
	/**
     * 字典集合国际化
     ...
     * @throws RuntimeException
     */
	public static void transformDict(List<Dictionary> dicts, HttpServletRequest request) throws RuntimeException {
        if(dicts==null){
            return;
        }
        for(Dictionary d:dicts){
            transformDict(d, request);
        }
    }
